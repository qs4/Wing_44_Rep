...
public class Points_Aus_s__calc_new5 extends JavaRule {
	...
	public void execute() throws Exception {
		for (Controlpoint_gen cpoint : getGraph().allInstances(Controlpoint_gen.class)) {
			...
			if (is_aus_s_flag == 1) {
				...
				NumberQuantity offset5 = null;
				if (is_right_flag == 1) {
					x_m = x_start.subtract(radius);
					offset5 = radius.multiply(Math.sin(Math.toRadians(45))).multiply(-1);
				} else if (is_right_flag == 0) {
					x_m = x_start.add(radius);
					offset5 = radius.multiply(Math.sin(Math.toRadians(45)));
				}
				
				NumberQuantity offset_positiv = radius.subtract(radius.multiply(Math.sin(Math.toRadians(45))));
//				System.out.println(Math.sin(Math.toRadians(45)));
				...
				if (Punktnummer == 1) {
					...
					x = x_start.add(offset5);
					z = z_start.subtract(offset_positiv);
				} else if (Punktnummer == 3) {
					x = x_m;
					z = z_m;
				} else if (Punktnummer == 4) {
					x = x_end.add(offset5);
					z = z_end.add(offset_positiv);
